...
import copy
from pysptools.distance import SAM, SID, NormXCorr, chebyshev


def calculate_distance(image, spec, distance_func):
    """
    Calculate the distance between each pixel in an xarray image and a spectral signature
    using a specified distance function from pysptools.
    
    Parameters:
    - image: xarray DataArray, the input image with dimensions ('ch', 'y', 'x')
    - spec: numpy array, the spectral signature with dimension ('ch',)
    - distance_func: function, one of the distance functions from pysptools (e.g., SAM, SID)
    
    Returns:
    - distance_map: xarray DataArray, the distance values for each pixel in the input image
    """

    if not isinstance(image, xr.DataArray) or not isinstance(spec, np.ndarray):
        raise ValueError("Invalid input types for image or spec.")
    # Initialize an empty numpy array for the distance values
    distance_values = np.empty((image.shape[1], image.shape[2]))
    
    # Iterate over each pixel in the image
    for y in range(image.shape[1]):
        for x in range(image.shape[2]):
            # Extract the spectrum of the current pixel
            pixel_spectrum = image[:, y, x].values
            
            # Calculate the distance between the pixel spectrum and the spectral signature
            distance = distance_func(pixel_spectrum, spec)
            
            # Store the distance value in the array
            distance_values[y, x] = distance
    
    # Convert the numpy array of distance values to an xarray DataArray
    distance_map = xr.DataArray(distance_values, dims=('y', 'x'), coords={'y': image.coords['y'], 'x': image.coords['x']})
