...
mfs = FilterBasedFeatureSelection(pd.concat([X_train_norm,X_val_norm]),pd.concat([y_train,y_val]),handle='mi')
selected_features = mfs.select_based_on_threshold(min_r2=0.50, r2_thresh=0.90)
open('feat_filter_hyper/filtered_features/mfs.txt', 'w').writelines(f"{item}\n" for item in selected_features)
